...
- 本次于 2026-09-21 复核官方来源：未发现已确认的美元 token 单价变化或符合纳入条件的新模型，保留全部 47 条价格记录。补充明确 DeepSeek 在中国公共假日全天按低谷价计费。人民币换算汇率仍为 `1 USD = 6.7080 CNY`。Kimi 模型专属价格链接仍跳转至不含价格表的总览页，GLM-5-Turbo 未出现在当前 Z.AI 价格表中；这五行保留旧值并标为 `unverified`（待核验）。
