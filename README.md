...
- Rechecked official sources on 2026-09-22: No confirmed USD token-price changes or eligible new models; all 47 price rows are retained. The CNY conversion rate remains `1 USD = 6.7080 CNY`. Kimi per-model pricing remains unavailable for verification (K3 redirects to an overview without price tables; K2.6 and K2.7 Code pages could not be fetched), and GLM-5-Turbo is absent from Z.AI pricing; these five rows retain previous values marked `unverified`.
